...
class DataField<T extends HelperFooterMessage & SpreadErrorMessage> {
  ...
  async get(props: HelperFooterMessage): Promise<T | undefined> {
    ...
    if (!this.identifier) {
      ...
      throw new Error(
        'Identifier not set. Call setIdentifier() before using get().'
      )
    }
...
export async function createDataField<
  T extends HelperFooterMessage & SpreadErrorMessage,
>(store: Record<string, T>, identifier: string): Promise<DataField<T>> {
