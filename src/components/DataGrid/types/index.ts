...
// The `DataGridTheme` interface that used to sit here was removed 2026-09-07.
// Its own docstring said it was "retained as a type for any consumer that
// historically imported it" — but it was never named in `src/index.ts`, so no
// consumer of the published package could reach it, and no file in this repo
// referenced it either. The values it described are CSS custom properties in
// `DataGrid.module.css` now; `DataGridStyles` below is the prop-shape contract
// that IS public.
